...
void calculate_pi(mpf_t& pi)
{
 ...
 // (mpf_cmp(x, x_prev) != 0)
    ...
    do
    {
        mpf_init_set(a_prev_variable, a_variable); 
...
//        cout << i << endl;
    ...
    while(i > 0 && mpf_cmp(a_variable, a_prev_variable) != 0);
...
void cube_root(mpf_t& x, const mpf_t a)
{
    ...
    while( i > 0 && mpf_cmp(prev, result) != 0);
...
void print_pi(mpf_t pi)
{
    char *pi_char = (char *) malloc(1002*sizeof(char));     // Size of the number of characters computed.
    mp_exp_t exp;

    mpf_get_str(pi_char, &exp, 10, 1001, pi);
